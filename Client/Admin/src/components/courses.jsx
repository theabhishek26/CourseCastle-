...
return(
  ...
  <Typography className="text-center md:text-4xl text-3xl font-semibold font-afacad">All Courses</Typography>
  ...
  return(
    <div className='courseCard border-2  p-2 rounded-xl bg-gradient-to-b from-orange-100 via-25% via-orange-200 to-80% mb-4 hover:bg-gradient-to-t'>
      ...
      <CardBody className="px-3 flex flex-col gap-3">
        ...
        <span class="material-symbols-outlined">sell</span><Typography className="mb-2 text-black text-2xl font-semibold">Price: <span className="text-green-600">{course.price}</span> </Typography>
        </div>
      </CardBody>
       
      <div className="btns flex justify-between px-2 gap-2">   
         ...
         <button className='course-card-btns bg-porange' onClick={()=>{
           fetch(server+'/admin/course/'+course._id,{
            method:'DELETE',
            headers:{'Authorization':'Bearer '+localStorage.getItem('token')}
           }).then((res)=>res.json().then((data)=>alert(data.message))).catch((err)=>alert(err))
         }}><span class="relative z-10 flex items-center md:text-lg text-md" >DELETE</span></button>
